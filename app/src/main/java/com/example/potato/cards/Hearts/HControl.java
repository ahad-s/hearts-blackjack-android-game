...
public class HControl extends FragmentActivity{
    ...
    // TODO: REPLACE ALL INSTANCES OF "suite" with "suit" :( im so stupid
    // TODO: LEARN THREADING AND FIX HOW CARDS COME INTO PLAYER'S HANDS WITH ANIMATION
...
//    ImageButton tempChild;
    ...
    static boolean blackQueenPlayed = false; // static since needs to be accessable from AI class

    private int humanCardDelay = 0; // needs to be set to 0 every time adjustAllViews() is called
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        totalWins = sharedPreferences.getInt("total_wins_hearts", MainMenu.DEFAULT_INT_VALUE);
        totalLosses = sharedPreferences.getInt("total_losses_hearts", MainMenu.DEFAULT_INT_VALUE);
        ...
        settingsButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent settingsIntent = new Intent(HControl.this, SettingsMenu.class);
                settingsIntent.putExtra("orientation" ,"horiz");
...
//        }
    ...
    public static ArrayList<Integer> getPlayerOrder(int startingPlayerTurn){
...
//        nextPassingRoundStart();
        ...
        // checks if the first 3 are the same suite and assigns values, otherwise the last one has to be the same suite -- THIS IS WHY WE USED LinkedHashMap to preserve order
...
//                                            toPlay = ((PlayerAI)players[turnOfPlayer - 1]).startIntelligentTurn(miniRoundTurnNumber, mustPlaySuite, tempHeartsBroken, players,currentCardsOnTable, playersTurnsLeft);
    ...
    boolean resetDelaysForFirstRound = false; // this is needed so that if player has first turn, AI keeps its delays
...
//        bot.getPlayerAnimation().setInterpolator(new LinearInterpolator());
        ...
        // if player is the first to play that round, reset all delays
        if (playerPlayedAlready && miniRoundTurnNumber == 1 && !resetDelaysForFirstRound) {
            resetDelaysForFirstRound = true;
            delay = 0;
        }
    ...
    private void endGamePopUp(boolean win){
        ...
        dialogBuilder.setNegativeButton("No", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Intent mainActivityIntent = new Intent(HControl.this, MainMenu.class);
...
//        scoreBoardIntent.putExtra("orientation", "horiz");
...
//            } catch (InterruptedException e) {}
    ...
    private void miniRoundStart(int turn){
        ...
        resetDelaysForFirstRound = false;
...
//                Log.d("TESTING", "TEST " + miniRoundTurnNumber);
            ...
            while (orderPlayIter.hasNext() && !playerPlayedAlready) {
                ...
                switch (turnOfPlayer) {
                    case 1:
                        // when player turn starts, this loop doesn't iterate anymore and ai turns will be initiated by the orderplay
...
//                        toPlay = ((PlayerAI) players[turnOfPlayer - 1]).startIntelligentTurn(miniRoundTurnNumber, mustPlaySuite, heartsBroken, players, currentCardsOnTable, orderPlay);
                        aiPlayerTurn(toPlay, players[turnOfPlayer - 1]);
                        break;
                }
...
//        adjustAllViews();
    ...
    // TODO: MAKE THIS LOOP IN ASYNC SO THAT CARDS ARE TRANSLATED TO PLACE ONE AFTER ANOTHER
    // TODO 2: WAIT FOR PLAYER TO CLICK SCOREBOARD TO START NEW ROUND
    // dynamically resets view for the beginning of a hand
    private void adjustAllViews(){
        for (int i = 0; i < cardSpaceLayout.getChildCount(); i++){
            final ImageButton tempChild = (ImageButton) cardSpaceLayout.getChildAt(i);
...
//            tempChild.setOnClickListener(null);
            ...
            Log.d("ADDING CARD0 ---- ", "ADDING " + getCardAssociatedWithImage(humanPlayer, tempChild).getStringValue() + " @@@@ " + humanCardDelay);





            // PROBLEM: ONLY EXECUTES THE LAST CARD IN THE ANIMATION
//            startPlayerCardAnimation(tempChild);


//            new Handler().postDelayed(new Runnable() {
//                @Override
//                public void run() {
//                    Log.d("ADDING CARD1 ---- ", "ADDING " + getCardAssociatedWithImage(humanPlayer, tempChild).getStringValue() + " @@@@ " + humanCardDelay);
//                    tempChild.startAnimation(pushRightIn);
//                    humanCardDelay++;
//                }
//            }, humanCardDelay * 125);

//            Log.d("ADDING CARD2 ---- ", "ADDING " + getCardAssociatedWithImage(humanPlayer, tempChild).getStringValue());
//            tempChild.startAnimation(pushRightIn);
//            humanCardDelay++;
    ...
    private void startPlayerCardAnimation(final ImageButton cardImageButton){
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                Log.d("ADDING CARD1 ---- ", "ADDING " + getCardAssociatedWithImage(humanPlayer, cardImageButton).getStringValue() + " @@@@ " + humanCardDelay);
                cardImageButton.startAnimation(pushRightIn);
            }
        }, ++humanCardDelay * 125);
    }
